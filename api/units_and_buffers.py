...
def condition_equality(condition1: db.WellCondition, condition2: db.WellCondition):
    ...
    epsilon = 1.01
    # make factors > 1 so our later multiplying makes sense
    if len(condition1.factors) != len(condition2.factors):
        return False
    
    for factor1 in condition1.factors:
        conc1 = unit_conversion(factor1.concentration, factor1.unit, factor1.chemical.density, factor1.chemical.molecular_weight, "M")
        # makes sure one is equal
        flag = False
        for factor2 in condition2.factors:
            ...
            if factor1.ph and factor2.ph and factor1.ph * epsilon < factor2.ph and factor1.ph / epsilon > factor2.ph:
                continue
            conc2 = unit_conversion(factor2.concentration, factor2.unit, factor2.chemical.density, factor2.chemical.molecular_weight, "M")         
